...
if flag==1
    ...
    for i=1:length(par.y)
        ...
        if size(par.y{i},2)~=0
            %[par.bin{i}]=Quantize(par.y{i}(:)',quantize, quantize.Mu(i), quantize.Sigm(i), quantize.bit(i), 1);
            [par.bin{i}] = VQ_Quantize(par.y{i}(:)',quantize.distr, quantize.Mu(i), quantize.Sigm(i), quantize.bit(i), quantize.dim(i), 1);
            par.bin{i}=uint32(par.bin{i})-1;
        end
    end
...
elseif flag==0
    ...
    for i=1:length(par.bin)
        ...
        [par.dec{i}] = VQ_Quantize(bin{i}(:)',quantize.distr, quantize.Mu(i), quantize.Sigm(i), quantize.bit(i), quantize.dim(i),0);
    end
